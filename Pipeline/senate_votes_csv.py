...
def senate(logname='senate_log.txt', min_session=101, max_session=116):
    ...
    log = open(logname, 'w')
    print("Started Senate")
    log.write("Trying to run the senate crawler on senate files.\n")
    log.write("Starting at time: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n")
    if not os.path.exists('./SenateCSV'):
        os.makedirs('./SenateCSV')
    for congress in range(min_session, max_session+1):
        for session in range(1,3):
            ...
            while(next_exists):
                ...
                try:
                    ...
                    continue
#            with open(('./SenateCSV/' + str(10*congress+session) + file_base_name), 'w') as f:
#                writer = csv.writer(f)
#                writer.writerows(TABLE)
#            log.write("Storing table " + (str(10*congress+session) + file_base_name) + "into csv file\n")
            year = 2*(congress-101) + (session-1) + 1989
            print(congress, session, year)
            with open(('./SenateCSV/' + str(10*congress+session) + file_base_name), 'w') as f:
                writer = csv.writer(f)
                writer.writerows(TABLE)
            log.write("Storing table " + (str(10*congress+session) + file_base_name) + " into csv file\n")
